...
- **Background Workers**: Celery with Redis for email delivery and report compilation.
    - **Reliability**: Implemented **Dead Letter Queue (DLQ)** task logging to capture and persist failed background operations.
...
- **Telemetry**: All events (Sent, Open, Click, Submit) are logged to the database and broadcasted in real-time to the instructor dashboard.
...
- **On-Demand Signing**: Reports are served via short-lived (1 hour) presigned MinIO URLs to ensure authorized access without exposing the storage backend.
